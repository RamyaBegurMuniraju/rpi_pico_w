...
static uint8_t adv_data[] = {
    ...
    // Name
    0x11, BLUETOOTH_DATA_TYPE_COMPLETE_LOCAL_NAME, 'P', 'i', 'c', 'o', ' ', 'c', 'o', 'n', 'n', 'e', 'c', 't', ' ', 'i', '2', 'c',
...
volatile uint8_t i2c_rx_data[32];
...
static void i2c0_irq_handler() {
    ...
    while (i2c_get_read_available(I2C_INSTANCE)) {
        ...
        if (i2c_rx_index >= sizeof(i2c_rx_data)) i2c_rx_index = 0;
        i2c_data_received = true;
    }

    // Handle read from master
...
void i2c_slave_init() {
    i2c_init(I2C_INSTANCE, 100000); // 100 kHz
    ...
    // Set slave address
    i2c0_hw->sar = I2C_SLAVE_ADDR;

    // Enable interrupts
    hw_set_bits(&i2c0_hw->intr_mask,
