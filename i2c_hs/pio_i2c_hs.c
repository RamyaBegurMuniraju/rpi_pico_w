...
#if 1
void pio_i2c_start(PIO pio, uint sm, uint pin_hs) {
    // Pure High-Speed START (no low-speed preamble)
    if (pin_hs < NUM_BANK0_GPIOS) gpio_put(pin_hs, true);  // enable HS current source / isolator if you have one
    // SDA ↓ while SCL ↑  then pull SCL low for first data bit
    pio_i2c_put_or_err(pio, sm, pio_i2c_prepare_instr(2));
    pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC1_SD0_HS]); // START
    pio_i2c_put_or_err(pio, sm, set_scl_sda_program_instructions[I2C_SC0_SD0_HS]); // SCL low
}
#endif
...
int pio_i2c_write_blocking(PIO pio, uint sm, uint pin_hs, uint8_t addr, uint8_t *txbuf, uint len) {
    ...
    if (pio_i2c_dma_ongoing(pio, sm)) {
	    printf("returning from here 1111\n");
    ...
    pio_i2c_put_or_err(pio, sm, pio_i2c_prepare_byte(((addr << 1) | 0), false));
    ...
    printf("returning from here 22222 err = %d\n", err);
    return err;
}
